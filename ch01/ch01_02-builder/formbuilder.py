...
class AbstractFormBuilder(metaclass=abc.ABCMeta):
    ...
    为了使用 abc 模块的 @abstractmethod 修饰器，
    我们必须把 AbstractFormBuilder 类的 metaclass 设置成 abc.ABCMeta

    如果某个类的 metaclass 是 abc.ABCMeta，那么该类就无法初始化了，
    只能把它当成抽象基类来用。把C++或Java代码移植到Python时，这么做
    确实很有用，不过会稍微增加运行期的开销。许多Python程序员都采用一种更为宽松的做法，
    那就是根本不使用metaclass，而是直接在文档中说明该类只能用作抽象基类。
    """
...
class HtmlFormBuilder(AbstractFormBuilder):
    ...
    def __init__(self):
        self.title = "HtmlFormBuilder"  # 提供默认标题
        # 表单的控件都保存在名为 items 的字典里
        # 字典的键是由 row 和 column 所构成的二元组，
        # 而值是控件的HTML代码。
        self.items = {}
...
pady="0.75m")""".format(name, row, column)
    ...
    def _canonicalize(self, text, startLower=True):
        """只要调用过 _canonicalize() 方法，那么后续调用时就可以直接从
        缓存中查找正则表达式了，不用再重新编译。
        """
...
def main():
    """
    创建两个表单，分别将其写入对应的文件中。
    在这种情况下，都会调用同一个表单创建函数（create_login_form()），
    每次调用时，传入与之相对应的builder对象
    """
